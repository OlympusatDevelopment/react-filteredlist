	...
	makeDisplayItem(item, items, selectedView, props) {
		...
		const languageFlag = item.languageLabel ? (<span className={`dl__displayItemFlag flag flag-us`}> </span>) : '',
			language = item.languageLabel ? (
				<p><span className="dl__displayItem--label">Language {languageFlag}</span></p>) : '',
			...
			cast = (item.cast && item.cast.length > 0) ? (<p><span className="dl__displayItem--label">Starring: </span> <span
				className="dl__displayItem--link">{item.cast.toString()}</span></p>) : '',
			director = (item.director && item.director.length > 0) ? (
				<p><span className="dl__displayItem--label">Directed By: </span> <span
					className="dl__displayItem--link">{item.director.toString()}</span></p>) : '',
			seasons = item.entityType === 'olyplat-entity-series' ? (<p style={{fontSize: '18px'}}>Seasons: 1 - 3</p>) : (
				<span style={{height: '21px', display: 'inline-block'}}> </span>),
		...
		return (<div className="dl__displayItemInner">
			<div className="dl__displayItemImgWrapper">
				...
				<span className="dl__featuredImgWrapper">
                            <img onClick={() => this.setState({imgIsOpen: true, currentImage: 0})}
			...
			<div>
				<div className="dl__displayItemInnerCol">
					...
					<div className="dl__displayItemMetaGroup">
						...
						<p><span className="dl__displayItem--label">Rating: </span> <span
							className="dl__displayItem--link">PG-TV</span></p>
					</div>
					<div className="dl__displayItemMetaGroup">
						{language}
						...
						<p><span className="dl__displayItem--label">Subtitles <span
							className={`dl__displayItemFlag flag flag-us`}> </span></span></p>
				...
				<div className="dl__displayItemInnerCol">
					<div className="dl__displayItem--pinBottom">
						...
						<p style={{
							...
							lineHeight: '0.7',
							padding: '5px 0'
						}}>{item.year}{languageFlag} <span style={{color: '#333', paddingLeft: '9px', fontWeight: '700'}}>HD</span>
						</p>
					</div>
				</div>
			</div>
			<div>
				<div className="dl__displayItemMetaGroup">
					...
					<div className="dl__displayItem--pinBottom">
						{seasons}
						<form>
							<button onClick={this.onAddToWatchlistClick} style={{marginRight: '9px'}}
											className="dl__displayItemButton dl__displayItemButton--stripe">Add to Watchlist
							</button>
